...
def test_VGGNet():
    ...
    train_ds, test_ds, class_names = data_load(r"D:\Python\PythonProject\Truck_identification\Final data\train",
                                              r"D:\Python\PythonProject\Truck_identification\new_data\test", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/VGGNet.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
    print('VGGNet test accuracy :', accuracy)
    ...
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="results/heatmap_VGGNet.png")
