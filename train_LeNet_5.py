...
def model_load(IMG_SHAPE=(224, 224, 3), class_num=2):
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(filters=6, kernel_size=5,
                      activation='sigmoid', input_shape=IMG_SHAPE),
...
def show_loss_acc(history):
    ...
    plt.xlabel('epoch')
    plt.savefig('results/result_LeNet.png', dpi=100)
...
def train(epochs):
    ...
    train_ds, val_ds, class_names = data_load(r"D:\Python\PythonProject\Truck_identification\Final data\train",
                                              r"D:\Python\PythonProject\Truck_identification\Final data\val", 224, 224, 32)
    print(class_names)
    # 加载模型
    ...
    model.save("models/LeNet_batch_32.h5")
    # 记录结束时间
    end_time = time()
